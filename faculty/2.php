...
<body>
	...
	<center>
		...
		<label class="label">Find your result:</label><br>
		<input type="text" name="regno" class="input" placeholder=" Enter Registration no:" required style="cursor:pointer;"><br><br>
		<input type="submit" name="submit" class="sub" style="cursor:pointer;"><br>
	...
	//{
	//	if($_SESSION["log"] == "success")
	//	{
	//		$session_life = time() - $_SESSION['log'];
	...
	{
		if($_COOKIE["log"] == "success")
		{
			include 'config.php';
		    if(isset($_POST['submit']))
			{
				$regg=mysqli_real_escape_string($db,$_POST['regno']);
			}
			else
			{
				$regg=0;
			}
			$sqlcommand="SELECT  `NAME OF THE STUDENT`,`SUB.CODE`, `SUBJECT NAME`, `Grade` FROM `sheet1` WHERE `REGD. NO`={$regg};";
	    ...
	    {
	    	Echo"<a href='index.php'style='text-decoration:none;color:red;'><h1 style='color:red;font-size:70px;'>Try Again ! Please login click here to login Again!</h1></a>";
	    }
	?>
	<!-- testing code-->
	<script>
	...
	if(isset($_POST['submit']))
	{
		echo "<div id='chartContainer' style='height: 350px; width: 60%;'></div>";
	}
	 ?>
	
	
	<!-- testing code-->
	<form method="POST" action="2.php"><input type=button name=print value='Print your result' onClick='window.print()'class='print'></form>
	<p>
